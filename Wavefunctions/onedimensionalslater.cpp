...
OneDimensionalSlater::OneDimensionalSlater(vec a, int N) : WaveFunction(a,N,1) {
    this->a = a(0);
    myorbital[0] = new HO0();
    myorbital[1] = new HO1();
    myorbital[2] = new HO2();
...
bool OneDimensionalSlater::newStep(mat &xnew, mat x,int &WhichParticle) {
    ...
    xnew.row(WhichParticle) = x.row(WhichParticle) + 1.0/NumberOfParticles*a*num(0);    //newstep(x)

    Rsd = 0;
    for(int j = 0;j<NumberOfParticles;j++) {
        Rsd += myorbital[j]->eval(xnew.row(WhichParticle),a, norm(xnew.row(WhichParticle)))*SlaterInverse(j,WhichParticle);
    }
    bool accept = false;
    if(Rsd*Rsd>num2(0)){
        accept = true;
...
mat OneDimensionalSlater::calculateSlater(mat x) {
    ...
    for(int i = 0;i<NumberOfParticles;i++) {
        for(int j = 0; j<NumberOfParticles;j++) {
            Slater(i,j) = myorbital[j]->eval(x.row(i),a, norm(x.row(i)));
        }
    }
    return Slater;
}
...
double OneDimensionalSlater::slaterAnalyticalLaplacianLog(arma::mat x) {
    ...
    for(int i =0; i<NumberOfParticles;i++) {
        for(int j = 0;j<NumberOfParticles;j++) {
            sum += myorbital[j]->laplacian(x.row(i),a, norm(x.row(i)))*SlaterInverse(j,i); //for some reason j and i are switched
        }
    }
    return sum;
}
        ...
        for (int j = 0; j<NumberOfParticles; j++) {
            if(j != i) {
                ...
                for( int l = 0; l< NumberOfParticles; l++) {
                    sum += oldSlater(l,j) * myorbital[l]->eval(x.row(i), a, norm(x.row(i)));
                }
